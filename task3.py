...
def get_lda_features2(paragraphs, num_topics):
...
if __name__ == "__main__":
    ...
    #第一个任务：在设定不同的主题个数T的情况下，分类性能是否有变化？
    num_topics = [5,10, 20, 100, 500, 1000, 3000]
    paragraph_lengths=1000
    for num in num_topics:
        print(f"num_topics: {num}")
        paragraphs, labels = extract_paragraphs(novels, paragraph_lengths)
        topic_matrix, vectorizer = get_lda_features(paragraphs, num)
    ...
    paragraphs, labels = extract_paragraphs(novels, paragraph_lengths,1)
    topic_matrix, vectorizer = get_lda_features2(paragraphs, num_topics)
    classify_and_evaluate(topic_matrix, labels, classifier='rf')
    print("finish 2")
    #3:不同的取值的K的短文本和长文本，主题模型性能上是否有差异？
    paragraph_lengths = [20, 100, 500, 1000, 3000]
    num_topics = 20  # T个主题
    for K in paragraph_lengths:
        print(f"Paragraph length: {K}")
        paragraphs, labels = extract_paragraphs(novels, K)
    ...
    paragraphs, labels = extract_paragraphs(novels, paragraph_lengths)
    topic_matrix, vectorizer = get_lda_features(paragraphs, num_topics)
    classify_and_evaluate(topic_matrix, labels, classifier='lr')
    classify_and_evaluate(topic_matrix, labels, classifier='nb')
    classify_and_evaluate(topic_matrix, labels, classifier='svm')
    classify_and_evaluate(topic_matrix, labels, classifier='rf')